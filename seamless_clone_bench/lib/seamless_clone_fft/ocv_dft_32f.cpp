...
#if defined(SC_OCV_DFT_NEON)
    p.opt.haveSimd = useSimd;
#else
    (void)useSimd;
...
static DftPlan& planFor(int len, bool inverse, bool scaled, bool useSimd) {
    ...
    auto ins = g_plans.emplace(key, makePlan(len, inverse, scaled, useSimd));
    return ins.first->second;
}

}  // namespace

void dft1dComplex32fInplace(float* interleaved, int len, bool inverse, bool scaled, bool useSimd) {
    if (len <= 0) {
        return;
    }
