...
try {
    & $Ninja ocl_test_findmax ocl_test_enhance ocl_test_enhance_only ocl_test_gws_style ocl_test_f32_to_f16
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} finally {
    Pop-Location
}
...
Write-Host "       .\scripts\run_f32_to_f16.ps1    (f32->f16 microbench)"
